...
public class VersionNodeFactoryTest {
    ...
    public void insertExistingTest() {
        ...
        final Relationship relation = mock(Relationship.class);
        List<Relationship> versions = new ArrayList<Relationship>() {
            {
                add(relation);
            }
        };
        when(artifactNode.getRelationships(ArtifactRelations.version, Direction.OUTGOING)).thenReturn(versions);
        Node versionNode = mock(VersionNodeDecorator.class);
        when(relation.getOtherNode(any(Node.class))).thenReturn(versionNode);
